...
public class ProjectSnapshotBean { 
	...
	public String getWorkspaceHtml(String workspaceID,int userGroupCode,int userCode,short reportType,int mode){
		...
		if(workspaceTreeVector.size() > 0){
			...
			if(countryCode.equalsIgnoreCase("IND")){
				...
				htmlDataStringBuffer.append("<TR align=\"left\" class=\"title\"><TH width=\"30%\">Node Name</TH><TH width=\"15%\">File Name</TH>" +
						"<TH width=\"11%\">Attribute Name</TH><TH width=\"9%\">Attribute Value</TH><TH width=\"9%\">Modify On</TH><TH width=\"6%\">Modify By</TH><TH width=\"9%\">Reason for Change</TH>" +
						"</TR>");
				...
				htmlDataStringBuffer.append("<Td align='left' style=\"padding-left:8px;\" colspan=\"4\"><font size=\"4\">"+dtows.getWorkSpaceDesc()+"</font></Td></tr></table>");
				htmlDataStringBuffer.append("<div style\"width:100%;border:1px solid;\"><TABLE class=\"datatable\" border=\"1px\" width='100%' cellspacing=\"0\" style=\"border:0px solid black\">");
				htmlDataStringBuffer.append("<TR align=\"left\" class=\"title\"><TH width=\"25%\">Node Name</TH><TH width=\"15%\">File Name</TH>" +
						"<TH width=\"10%\">Attribute Name</TH><TH width=\"8%\">Attribute Value</TH><TH width=\"8%\">Modify On</TH>"
						+ "<TH width=\"8%\">Eastern Standard Time</TH><TH width=\"7%\">Modify By</TH><TH width=\"9%\">Reason for Change</TH>" +
						"</TR>");
				htmlDataStringBuffer.append("<TR class=\"none\">");
					htmlDataStringBuffer.append("<TD align='left'width=\"30%\" class=\"title\"><B>"+(String) firstNodeObj[0] +"</B></TD>");	
	...
	private StringBuffer getTableChildStructure(Integer nodeId, TreeMap<Integer, ArrayList<Integer>> childNodeHS, Hashtable<Integer, Object[]> nodeDtlHS){
		...
		tableHtml.append("<TD align='left' width=\"10%\" class=\"title\" valign=\"middle\" >"+nodeName+"</TD>");
	...
	private StringBuffer createNodeAttrTRsForTable(ArrayList<DTOWorkSpaceNodeAttrDetail> attrList) {
		...
		if(attrList.size()>0){
				...
				DTOWorkSpaceNodeAttrDetail dtoWorkSpaceNodeAttrDetail = iterator.next();
					...
					if(attrValue!=null ){
						...
						if(attrValue.contains("/")){
							String attrVal[]=attrValue.split("/");
							finalAttrValue=attrVal[0]+"/"+"\n"+attrVal[1];
							attrValue=finalAttrValue;
						}
					}
					else{
						attrValue = "";
				...
				if(countryCode.equalsIgnoreCase("IND")){
					tableHtml.append("<TD valign=\"middle\" width=\"25%\">"+dtoWorkSpaceNodeAttrDetail.getAttrName() +"</TD>");
					...
					tableHtml.append("<TD valign=\"middle\" width=\"15%\">"+((dtoWorkSpaceNodeAttrDetail.getModifyBy() != 0 && dtoWorkSpaceNodeAttrDetail.getModifyBy() != 1)?allUsers.get(dtoWorkSpaceNodeAttrDetail.getModifyBy()).getUserName():"-")+"</TD>");	
					...
					tableHtml.append("<TD valign=\"middle\" width=\"18%\">"+((dtoWorkSpaceNodeAttrDetail.getModifyBy() != 0 && dtoWorkSpaceNodeAttrDetail.getModifyBy() != 1)?allUsers.get(dtoWorkSpaceNodeAttrDetail.getModifyBy()).getUserName():"-")+"</TD>");
	...
	if(!countryCode.equalsIgnoreCase("IND")){
		attrHistoryTable.append("<tr class='title'><th width=24%>Attribute Value</th><th width=24%>Modify By</th><th width=24%>Modify On</th><th width=24%>Eastern Standard Time</th><th width=24%>Reason for Change</th></tr>");
		}
	else{
		attrHistoryTable.append("<tr class='title'><th width=25%>Attribute Value</th><th width=25%>Modify By</th><th width=25%>Modify On</th><th width=25%>Reason for Change</th></tr>");
	}
