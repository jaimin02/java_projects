package com.docmgmt.struts.actions.labelandpublish.CA;
...
public class WorkspacePublishService_CA{
	...
	public WorkspacePublishService_CA() {
		...
		if (childNodes.size() == 0) {
			...
			if (AllNodesofHistory.contains(nodeId)) {
				...
				if (childAttrId.size() != 0) {
					...
					if (pathToCreate.matches(".*/m1/ca/.*-cover.*")) {
					...
					for (int i = 0; i < childAttrId.size(); i++) {
						...
						if (attrName.equalsIgnoreCase("operation")
							...
							for (int j = 0; j < modifiedFile.size(); j++) {
								...
								if (pathToCreate.contains("m1/ca")) {
									query.append("../../../");
						...
						if (attrName.equalsIgnoreCase("xlink:href")) {
							if (operationValue.equals("delete")) {
								...
								System.out.println("Atrribute value:before:"+attrValue);
								if (attrValue.trim().charAt(0) == '/') {
								...
								System.out.println("Atrribute value:"+attrValue);
	...
	private void writeToXmlFile(String stype, PublishAttrForm publishForm)
		...
		if (stype.equals("ca")) {
			...
			//String docTypeDeclaration = "<!DOCTYPE au:au-backbone SYSTEM \"../../util/dtd/au-regional.dtd\">";
			//String xslStyleSheetDeclaration = "<?xml-stylesheet href=\"../../util/style/ca-regional.xsl\" type=\"text/xsl\"?>";
			
			//String xslStyleSheetDeclaration = "<?xml-stylesheet type=\"text/xsl\" href=\"../../util/style/au-regional.xsl\"?>";
			
			String XmldtdVersionDeclaration = "<hcsc_ectd schema-version=\"2.2\"  xmlns=\"hcsc_ectd\"  xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"  xsi:schemaLocation=\"hcsc_ectd ../../util/dtd/ca-regional-2-2.xsd\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" >";
			...
			out.write("\n" + "<ectd-regulatory-transaction-information>");
			...
			out.write("\n" +"<product-name>" + publishForm.getProdName() + "</product-name>");	
			
			out.write("\n" + "<dossier-identifier>"+ publishForm.getDossierIdentifier() +"</dossier-identifier>");
			
			out.write("\n" + "<dossier-type>"+ publishForm.getDossierType() + "</dossier-type>");
			
			out.write("\n" + "<regulatory-activity-type>"+ publishForm.getRegActType() +"</regulatory-activity-type>");
			
			out.write("\n" + "<regulatory-activity-lead>" + publishForm.getRegActLead() +"</regulatory-activity-lead>");
			...
			String[] desvalue=publishForm.getSequenceDescriptionValue().split("#");
			if(publishForm.getSeqDescFlag().equalsIgnoreCase("twoDate")){
				
				out.write("\n" + "<sequence-description>"+publishForm.getSequenceDescription()+" "+desvalue[0].trim() + " to "+desvalue[1].trim()+"</sequence-description>");
			}
			else if(publishForm.getSeqDescFlag().equalsIgnoreCase("mixDesc")){
				
				out.write("\n" + "<sequence-description>"+publishForm.getSequenceDescription()+" "+ desvalue[0] +" dated "+desvalue[1].trim()+"</sequence-description>");
			}
			else if(publishForm.getSeqDescFlag().equalsIgnoreCase("desc")){
				
				if(publishForm.getSequenceDescription().equalsIgnoreCase("Year, list of change number")){
					
					out.write("\n" + "<sequence-description>"+ desvalue[0] +"</sequence-description>");
				}
				else{
					out.write("\n" + "<sequence-description>"+publishForm.getSequenceDescription()+" "+ desvalue[0] +"</sequence-description>");	
				}
				
			}
			else if(publishForm.getSeqDescFlag().equalsIgnoreCase("singleDate")){
				
				out.write("\n" + "<sequence-description>"+publishForm.getSequenceDescription().trim()+" "+desvalue[0].trim()+"</sequence-description>");
			}
			else{
				out.write("\n" + "<sequence-description>"+publishForm.getSequenceDescription()+"</sequence-description>");
			}
			...
			out.write("\n" + "</ectd-regulatory-transaction-information>");// End of envelopes
		}

		else if (stype.equals("cam2-m5")) {
			...
			String camd5 = md5.getMd5HashCode(absolutePathToCreate + "/m1/ca"
					+ "/ca-regional.xml");
			System.out
					.println("CheckSum ="
							+ "<leaf xml:lang=\"en\" checksum-type=\"md5\" modified-file=\"\" operation=\"new\" application-version=\"\" xlink:href=\"m1/ca/ca-regional.xml\" checksum=\""
							+ camd5
			...
			out
					.write("<leaf xml:lang=\"en\" checksum-type=\"md5\" operation=\"new\" application-version=\"\" xlink:href=\"m1/ca/ca-regional.xml\" checksum=\""
							+ camd5
							+ "\" ID=\"node-999\" xlink:type=\"simple\">");
			out.write("<title>");
			out.write("CA Module 1");
	...
	public String generateStudyIndetifier(int firstnodeId, String wsId) {
		...
		studyidendata += "util/style/ich-stf-stylesheet-2-2a.xsl\"?>\n";
