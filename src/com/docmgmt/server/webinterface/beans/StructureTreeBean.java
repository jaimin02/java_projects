...
public class StructureTreeBean { 
...
//			Hashtable parentNodeHS = new Hashtable();
			...
			if(firstNode != null){
				...
				htmlSB.append("<input type= \"checkbox\" name ='CHK_" +firstNode.intValue()+"' value='CHK_" +firstNode.intValue()+"'   style= \"border:0px\" >");
				htmlSB.append("<IMG id='folder" + firstNode.intValue() + "' src=\"images/closed.gif\" border=\"0\">" + "<a  href=\"StructureNodeAttrAction.do?requestId="+ ClientRequestID.WORKSPACE_NODE_RIGHTS +"&nodeId="+firstNode+"&displayName="+((String)firstNodeObj[0])+"&leaf=\"yes\"\" target=\"attributeFrame\" >" + ((String)firstNodeObj[0]).replaceAll(" ","&nbsp;") + "</a><BR>\n");
...
//			System.out.println("htmlSB " + htmlSB.toString());
	...
	private StringBuffer getChildStructure(Integer nodeId, TreeMap childNodeHS, Hashtable nodeDtlHS) {
		...
		if(childList.size() > 0){
            ...
            htmlStringBuffer.append("<input type= \"checkbox\" name = 'CHK_" +nodeId.intValue()+"' value='CHK_" +nodeId.intValue()+"' style= \"border:0px\">");
            htmlStringBuffer.append("<IMG id='folder" + nodeId.intValue() + "' src=\"images/closed.gif\">" + "<a  href=\"StructureNodeAttrAction.do?requestId="+ ClientRequestID.WORKSPACE_NODE_RIGHTS +"&nodeId="+nodeId+"&displayName="+displayName+"&leaf=\"yes\"\" target=\"attributeFrame\" >" + NodeName.replaceAll(" ","&nbsp;") + "</a><BR>\n");
...
//            }else if(getUserType().equalsIgnoreCase("WA")) {
            	htmlStringBuffer.append("<IMG  src=\"images/empty.gif\"><a  href=\"StructureNodeAttrAction.do?requestId="+ ClientRequestID.WORKSPACE_NODE_RIGHTS +"&nodeId="+nodeId+"&displayName="+displayName+"&leaf=\"yes\"\" target=\"attributeFrame\" >" + FolderName.replaceAll(" ","&nbsp;") + "</a><BR>\n");
//            }
		}
