	...
	<table width="95%" style="padding-bottom: 4px" align="center">
		<tr>
			<td align="center" class="title label"><font
				style="font-style: normal; font-weight: bold; font-size: 14px;">All Comments
			</font></td>
		</tr>
	</table>
	<div
	...
	<table width="100%" class="datatable" align="center">
		...
		<tbody>
			...
			<s:iterator value="sentComments" status="status">
			 ...
			 <s:if test='%{#status.count !=1 && ResolverFlag == "Y"}'> 
			<tr bgcolor="#5B89AA">
     			 <td  colspan="9" ></td>
  			 </tr>
			</s:if> 
				...
				<tr
					...
					<!-- <s:if test='%{ResolverFlag == "Y"}'>style="background-color:#5B89AA;"</s:if>  --> 
				...
				<s:if test ="#session.countryCode != 'IND'">
					...
					<td><s:if test='typeFlag==null || typeFlag=="C"'></s:if><s:else><s:property value="ResolverName"/></s:else></td>
					<td><s:if test='typeFlag==null'>Open</s:if>
						<s:elseif test='typeFlag=="C"'>Close</s:elseif>
						<s:else>Resolved</s:else>
					</td>
