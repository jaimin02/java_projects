...
<script type="text/javascript">
	...
	function getProjectSnapshot(reportType){
		...
		var urlOfAction="ShowSiteWiseProjectSnapshot_ex.do?mode="+mode;//mode=1  for latest  
	...
	{	
		...
		$.ajax(
		{			
			url: 'ShowSiteWiseProjectSnapshot_ex.do?workSpaceId=' + workSpaceId+'&mode='+mode,
			beforeSend: function()
			{
					$('#ProjectSnapshotDiv').html("<img src=\"images/loading.gif\" alt=\"loading ...\" />");								
			},
	...
	{			
			//debugger;
			var AttrId = $('#SelectedAttribute').val();
			var WorkspaceId =$('#workSpaceId').val();
							
			$.ajax(
			{			
				url: 'GetUserModuleWiseAttributeDetail_ex.do?attrId='+AttrId+'&wsId=' + WorkspaceId,
		  		success: function(data) 
		  		{
		    		...
		    		if(data.length > 0)
			    	{
		    			...
		    			opt = '<select name="attributeValue">'+
		    					opt +
	  						  '</select>';
		    			document.getElementById('AttributeValueList').innerHTML = opt;
...
<table width="100%">
	<tr>
		...
		<tr>
			<td class="title" align="right" width="25%"
				style="padding: 2px; padding-right: 6px;">Attribute Name &nbsp;</td>
			<td align="left"><select name="SelectedAttribute" id="SelectedAttribute" onchange="getAgencies();">
				<option value="-1">Select Attribute</option>
				<s:iterator value="getSelectedAttrDetail">
					
						<option value="<s:property value="attrId"/>"><s:property
		...
		<tr>
				...
				<td>	
				<div id="AttributeValueList"><SELECT name="attributeValue">
