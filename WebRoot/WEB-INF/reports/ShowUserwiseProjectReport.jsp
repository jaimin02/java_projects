...
<table id = "ScriptTable" class="datatable paddingtable audittrailtable" width="100%" align="center">
	<thead>
		<tr>
			...
			<th style="border: 1px solid; border-color:black; width: 25%;">Project name</th>
			<th style="border: 1px solid; border-color:black; width: 40%;">${ lbl_nodeName }/${ lbl_folderName }</th>
			<th style="border: 1px solid; border-color:black; width: 15%;"><s:property value=""/>Stage Assigned</th>
	...
	<tbody>
		<s:iterator value="getUserForProjectDeatil" status="status">
			<tr class="<s:if test="#status.even">even</s:if><s:else>odd</s:else>">
				...
				<%-- <s:property value="nodeDisplayName" /> [<s:property value="folderName" />] --%>
				<s:if test="completedStageId ==0 || tranNo==0">
					<s:property value="nodeDisplayName" /> [ <s:property value="folderName" /> ]
				</s:if>
				<s:else>
				<a href="javascript:void(0);"
							onclick="return fileOpen('openfile.do?workSpaceId=<s:property value="WorkSpaceId"/>&nodeId=<s:property value="nodeId"/>&tranNo=<s:property value="tranNo"/>&fileName=<s:property value="fName"/>&baseWorkFolder=<s:property value="baseWorkFolder"/>');">
						<s:property value="nodeDisplayName" /> [ <s:property value="fName" /> ] </a>
				</s:else>
				...
				<td style="border: 1px solid; border-color:black;"><s:property value="stageDesc" /></td>
				<%-- <td style="border: 1px solid; border-color:black;"><s:property value="attrName" /></td> --%>
				<td style="border: 1px solid; border-color:black;">
					<s:if test="completedStageId==10 && fileType=='SR' ">Sign off Completed</s:if>
					<s:else>
						<s:if test="completedStageId ==0">Pending</s:if>
