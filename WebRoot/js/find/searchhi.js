...
/*
 * Added By Rahul Goswami On 20-Apr-2011
 * Unhighlight all tree node with passed color.
 * */
function unhighliteAllTreeNode(node,color)
{
	...
	if (node.hasChildNodes) 
	{
		...
		for (hi_cn=0;hi_cn<node.childNodes.length;hi_cn++) 
		{
			$(node.childNodes[hi_cn]).css('background-color', color );
			unhighliteAllTreeNode(node.childNodes[hi_cn],color);
		}
	}
	if (node.nodeType == 3) { // text node
		pn = node.parentNode;
...
function localSearchHighlight(searchStr,target, tryQ, doc) {
		...
		qsip = qsa[i].split('=');
			...
			for(p=0;p<phrases.length;p++) {
				...
				if( p % 2 == 0 ) words = phrases[p].replace(/([+,()]|%(29|28)|\W+(AND|OR)\W+)/g,' ').split(/\s+/);
	                	...
	                	for (w=0;w<words.length;w++) {
							if( words[w] == '' ) continue;
							if(target != null && typeof(tryQ) != 'undefined'){
								highlightWord(target,words[w],doc);
							}else{
								highlightWord(doc.getElementsByTagName("body")[0],words[w],doc);
							}
...
function SearchHighlight(doc,loc) {
     ...
     if( doc.createElement ) {
                ...
                || doc.referrer.match(/\/search\/web\//) ) ) 
          {
	          ...
	          localSearchHighlight(loc.search,null, 0, doc);
          } else {
	          ...
	          localSearchHighlight(locsearch,null, 1, doc);
          }
