...
//		this.isRead = false; // Lazy content not yet read
	...
	_getInnerHtml: function() {
		...
		if(urlStr){									
			var StartIndex=urlStr.indexOf('=', 0);
			var EndIndex=urlStr.indexOf('&', 0);
			var nodeId=urlStr.substr(StartIndex+1,EndIndex-StartIndex-1);			
			res +=  "<a href='#' action='"+this.data.action+"' name='"+nodeId+"'  class='button'  class='" + opts.classNames.title + " '" + tooltip + ">" + this.data.title + "</a>";			
		}else if(actionStr){
			res +=  "<a href='#' action='"+this.data.action+"' class='button'  class='" + opts.classNames.title + " '" + tooltip + ">" + this.data.title + "</a>";
		}else{			
			res +=  "<a href='#'  class='" + opts.classNames.title + " '" + tooltip + ">" + this.data.title + "</a>";			
		}
		//res +=  "<a href='#' class='button'  class='" + opts.classNames.title + " '" + tooltip + ">" + this.data.title + "</a>";
		return res;
	},
