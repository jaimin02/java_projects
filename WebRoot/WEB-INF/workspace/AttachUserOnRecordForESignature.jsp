        ...
        stop: function (e, ui) {
            ...
            $(this).find("tr").each(function (index) {
                if (index > 0) {
                    ...
                    var userCode=$(this).find("input[type='hidden']")[0].value;
                    var wsId= '<s:property value="WorkspaceId"/>';
        			$.ajax
    				({			
    					url: "updateMaintainedSeqForESign_b.do?WorkspaceId="+wsId+"&nodeId="+<s:property value="nodeId"/>+"&userCode="+userCode+"&TranNo="+index,
    					beforeSend: function()
    					{											
    							//						
    					},
...
//var userType;
	...
	$j(document).ready(function() { 
		...
		/* fSelect 1.0.1 - https://github.com/mgibbs189/fselect */
		 ...
		 function getUsers() {
			 ...
			 $j.ajax({				
				 url: "showUserGroupDtlForESignature_ex.do?WorkspaceId="+wsId+"&nodeId="+<s:property value="nodeId"/>+"&userGroupId=" + userGroupId,
						beforeSend: function()
						{
					    	 ...
					    	 for(i=0; i<usersArr.length;i++)
						       {
					    		...
					    		opt = '<select id="userListDropdown" name="userCode" class="temp" multiple="multiple" >'+
					    		opt +
				  				  '</select>';
					    		   document.getElementById('showUserDtl').innerHTML = opt;
	...
	return true;
				...
				if(ok==true){
					...
					remark = remark.trim();
					 if (remark != null && remark != ""){ 
						 $('#remark').val(remark);
						 $.ajax( {
								url:'RemoveAssignedUserRightsForRecordOnESignature_ex.do?WorkspaceId='+wsId+'&stageIds='+stageIds+'&userCode='+userCode+'&nodeId='+nodeId+'&remark='+remark,
								beforeSend : function() {
		...
		function openlinkEdit(buttonName,stageIds,userCode,userGroupCode,nodeId,roleCode){
 			...
 			str="ChangeRecordRightsForESignature_b.do?WorkspaceId="+wsId+"&userGroupId="+userGroupCode+"&stageIds="+stageIds+"&userCode="+userCode+"&nodeId="+nodeId+"&RightsType="+modulewiserights+'&roleCode='+roleCode;
		...
		function moduleUserHistory(nodeId)
		{
		 ...
		 str="ShowAssignModuleUserDetailHistoryForESignature_b.do?docId="+workspaceId+"&recordId="+<s:property value="nodeId"/>;
...
<div class="boxborder"><div class="all_title"><b style="float:left:">Attach Signatories On New Created Document</b></div>

<div style="border: 1px; border-color: #5A8AA9; border-style: solid; padding-bottom: 15px; border-top: none;"
	align="center"><br>
<div align="left">

<s:form action="SaveUserOnRecordForESignature" method="post" name="addUsertoProjectForm">
...
<div id="imp" style="float: right; margin-right: 10px; margin-top: -10px;">
	...
	<table width="100%">
		...
		<!-- <tr>
			<td align="right" id="Sequence" style="padding: 2px; padding-right: 8px;" class="title" width="50%">
				<input style="height: 13px; width: 13px;"type="checkbox" name="maintainSeq" id="maintainSeq" value="Y" >
			</td>
...
<div style="max-height: 400px; overflow: auto; border: 1px solid; width: 98%; margin-left: 12px;"
	align="center">
<table align="center" style="width:100%;" id="tblLocations" class="datatable" border="1px solid">
	<thead>
		<tr>
			<th>#</th>
			<th>Username</th>
	...
	<tbody>
		...
		<s:iterator value="getUserRightsDetail" id="getUserRightsDetail"
			...
			<tr class="even none">
				...
				<s:if test="#prevUserName != userName">
					<td rowspan="<s:property value="#userCount"/>"><s:property value="#counter" /></td>
					<td rowspan="<s:property value="#userCount"/>">
						<s:property value="userName" />
						<input type="hidden" name="uCode" value="<s:property value='userCode' />"></td>
				</s:if>
			...
			<s:else>-</s:else>
				...
				-
				 ...
				 <s:elseif test="(userTypeCode=='0002' || userTypeCode=='0003') &&  userFlag == 'N'">
				...
				<td>		
					...
					userTypeCode : <s:property value="userTypeCode"/> --%>
					<s:if test="#session.username == userName && userTypeCode=='0002'">
						-
					</s:if>
					<s:elseif test="(userTypeCode=='0002' || userTypeCode=='0003') &&  userFlag == 'N'">
...
function setValues(selectObject) {
	debugger;
	/* if(selectObject.selectedIndex != -1){
		...
		//if(order.includes(selectObject.options[selectObject.selectedIndex].value, 0))
		if(order.includes(value)){			
			order.splice(order.indexOf(value), 1);
		}
		else{ */
			order.push(selectObject.options[selectObject.selectedIndex].value);
			//order.push(value);
		//}
		$('#result').html(order);
	//}
...
/* document.getElementById("userListDropdown").onClick = function() {
