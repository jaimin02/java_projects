...
public class RepositoryCopyWorkspaceTreeBean { 
	...
	public RepositoryCopyWorkspaceTreeBean(){ 
		...
		if(workspaceTreeVector.size() > 0){
			...
			if(firstNode != null){
				Object[] firstNodeObj = (Object[]) nodeDtlHS.get(firstNode);
	             ...
	             htmlSB.append("<input type= \"checkbox\" name ='CHK_" + firstNode.intValue() + "' value='CHK_" + firstNode.intValue() + "' style= \"border:0px\" onClick = \" return filledHiddenValue('" + firstNode.intValue() + "');\">");
	...
	private StringBuffer getChildStructure(Integer nodeId, TreeMap childNodeHS, Hashtable nodeDtlHS) {
		...
		if(childList.size() > 0){
	     ...
	     //    htmlStringBuffer.append("<input type= \"checkbox\" name = 'CHK_" + nodeId.intValue() + "' value='CHK_" + nodeId.intValue() + "'  style= \"border:0px\" onClick = \" return filledHiddenValue('" + nodeId.intValue() + "');\">");
	         htmlStringBuffer.append("<IMG id='folder" + nodeId.intValue() + "' src=\"images/closed.gif\">" + NodeName +  "<BR>\n");
			...
			htmlStringBuffer.append("</SPAN>\n");
				...
				htmlStringBuffer.append("<input type= \"checkbox\" name ='CHK_" + nodeId.intValue() + "' value='CHK_" + nodeId.intValue() + "' style= \"border:0px\" onClick = \" return filledHiddenValue('" + nodeId.intValue() + "');\">");
           		htmlStringBuffer.append("<IMG  src=\"images/empty2.gif\">" + FolderName.replaceAll(" ","&nbsp;") + "</a><BR>\n");
           	//<a  href=\"workspaceNodeAttrAction.do?requestId="+ ClientRequestID.WORKSPACE_NODE_ATTR +"&nodeId="+nodeId+"&leaf=\"yes\"\" target=\"attributeFrame\" >	
