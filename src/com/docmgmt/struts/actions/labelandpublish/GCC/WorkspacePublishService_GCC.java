package com.docmgmt.struts.actions.labelandpublish.GCC;
...
public class WorkspacePublishService_GCC {
	  public Writer writer;
	    ...
	    public WorkspacePublishService_GCC()
	    {
	    ...
	    public void workspacePublish(String workspaceId,PublishAttrForm publishForm,HttpServletRequest request1,String wsDesc)
		 {     
		    try {
	            ...
	            System.out.println("Step -1 Created");
	            
	            if(projectPublishType == 'P'){
	            	//get all nodes and its parent nodes where file attached
	            	AllNodesofHistory=docMgmtInt.getAllNodesFromHistoryForRevisedSubmission(wsId,labelNo);
	            }
	            else if(projectPublishType == 'M'){
	            	//In case of Manual Mode getting all parents of the selected nodes
	            	AllNodesofHistory=docMgmtInt.getWorkspaceTreeNodesForLeafs(workspaceId, leafIds);
	            }	              
	            //Copy Util folder
	            addutilFolder(stype);	            
	            if(stype.equals("gcc"))
			...
			if (childNodes.size() == 0) {
				...
				if (AllNodesofHistory.contains(nodeId)) {
					...
					if (childAttrId.size() != 0) {
						...
						if (pathToCreate.matches(".*/m1/gc/.*-cover.*")) {
						...
						for (int i = 0; i < childAttrId.size(); i++) {
							...
							if (attrName.equalsIgnoreCase("operation")
								...
								for (int j = 0; j < modifiedFile.size(); j++) {
									...
									if (pathToCreate.contains("m1/gc")) {
										query.append("../../../");
										query.append("" + lastPubVersion + "");

										query.append("/m1/gc/gc-regional.xml#");
									} else {
										query.append("../");
										query.append("" + lastPubVersion + "");
							...
							if (attrName.equalsIgnoreCase("xlink:href")) {
								if (operationValue.equals("delete")) {
									...
									attrValue = attrValue.replaceAll("m1/" + "gc"
											+ "/", "");
		...
		private String copyFileforPublish(PublishAttrForm publishForm,
			...
			if (fileNameDtl.size() > 0) {
				...
				System.out.println("Folder Struct="+folderStructure);
					
				folderStruct = new File(sourceFolderName.trim()
						+ folderStructure.trim() + "/" + fileName);
				
				System.out.println("Folder Name(Absolute="+folderStruct.getAbsolutePath());
		...
		private void createSubFolders(String pathToCreate, String folderName) {
			...
			//System.out.println("Path To Create="+relativePathToCreate.indexOf(str, fromIndex));
		...
		//dtoSubmissionMst.getRegionalDTDVersion()==null || dtoSubmissionMst.getRegionalDTDVersion().trim().equalsIgnoreCase("")
		private void addutilFolder(String stype) {
			try {
				...
				if(stype.equals("gcc") && (sversion==null || sversion.trim().equalsIgnoreCase(""))){
				...
				else if(stype.equals("gcc") && sversion.equals("12") ){
					File inDtdFile = new File(baseWorkDir.getAbsoluteFile()
							+ "/util_gcc/util/dtd");
				...
				else if(stype.equals("gcc") && sversion.equals("15"))
	            {
					File inDtdFile = new File(baseWorkDir.getAbsoluteFile()
							+ "/util_gcc15/util/dtd");
		...
		private void writeToXmlFile(String stype, PublishAttrForm publishForm)
			...
			if (stype.equals("gcc")) {
				...
				String docTypeDeclaration = "<!DOCTYPE gc:gc-backbone SYSTEM \"../../util/dtd/gc-regional.dtd\">";
				String xslStyleSheetDeclaration = "<?xml-stylesheet type=\"text/xsl\" href=\"../../util/style/gc-regional.xsl\"?>";
				if(sversion==null || sversion.trim().equalsIgnoreCase("")){
					 XmldtdVersionDeclaration = "<gc:gc-backbone xmlns:gc=\"http://sfda.gov.sa\" xmlns:xlink=\"http://www.w3c.org/1999/xlink\" dtd-version=\"1.0\">";
				}
				else if(sversion.equals("12")){
					 XmldtdVersionDeclaration = "<gc:gc-backbone xmlns:gc=\"http://sfda.gov.sa\" xmlns:xlink=\"http://www.w3c.org/1999/xlink\" dtd-version=\"1.0\">";
				}
				else if(sversion.equals("15")){
					XmldtdVersionDeclaration = "<gc:gc-backbone xmlns:gc=\"http://sfda.gov.sa\" xmlns:xlink=\"http://www.w3c.org/1999/xlink\" dtd-version=\"1.1\">";	
				}
				 ...
				 */
				ArrayList<DTOSubmissionInfoGCCSubDtl> cmsDtl = docMgmtInt
						.getWorkspaceCMSSubmissionInfoGCC(publishForm
								.getSubmissionId());
				...
				if (cmsDtl.size() > 0) {
					...
					DTOSubmissionInfoGCCSubDtl dtoRms = docMgmtInt
							.getWorkspaceRMSSubmissionInfoGCC(publishForm
									.getSubmissionId());
				...
				if (publishForm.getRMSSelected() == 'Y') {
					...
					// ATC
					out.write("\n" + "<atc>"
							+ publishForm.getAtc() + "</atc>");
					
					out.write("\n" + "<submission ");
					out.write("type=\""
							+ publishForm.getSubmissionType() + "\"/>");
					if(sversion.equals("15")){
						
						System.out.println("Submission unit type in serverice:"+publishForm.getSubmissionUnitType());
				...
				String eumd5 = md5.getMd5HashCode(absolutePathToCreate + "/m1/gc"
						+ "/gc-regional.xml");
				System.out
						.println("CheckSum ="
								+ "<leaf xml:lang=\"en\" checksum-type=\"md5\" modified-file=\"\" operation=\"new\" application-version=\"\" xlink:href=\"m1/gc/gc-regional.xml\" checksum=\""
								+ eumd5
				...
				out
						.write("<leaf xml:lang=\"en\" checksum-type=\"md5\" operation=\"new\" application-version=\"\" xlink:href=\"m1/gc/gc-regional.xml\" checksum=\""
								+ eumd5
								+ "\" ID=\"node-999\" xlink:type=\"simple\">");
				out.write("<title>");
				out.write("GCC Module 1");
