package com.docmgmt.struts.actions.labelandpublish.AU;
...
public class WorkspacePublishService_AU {
	...
	public WorkspacePublishService_AU() {
		...
		if (childNodes.size() == 0) {
			...
			if (AllNodesofHistory.contains(nodeId)) {
				...
				if (childAttrId.size() != 0) {
					...
					if (pathToCreate.matches(".*/m1/au/.*-cover.*")) {
					...
					for (int i = 0; i < childAttrId.size(); i++) {
						...
						if (attrName.equalsIgnoreCase("operation")
							...
							for (int j = 0; j < modifiedFile.size(); j++) {
								...
								if (pathToCreate.contains("m1/au")) {
									query.append("../../../");
									query.append("" + lastPubVersion + "");

									query.append("/m1/au/au-regional.xml#");
								} else {
									query.append("../");
									query.append("" + lastPubVersion + "");
	...
	private void writeToXmlFile(String stype, PublishAttrForm publishForm)
		...
		if (stype.equals("au")) {
			...
			String xslStyleSheetDeclaration = "<?xml-stylesheet href=\"../../util/style/au-regional.xsl\" type=\"text/xsl\"?>";
			
			//String xslStyleSheetDeclaration = "<?xml-stylesheet type=\"text/xsl\" href=\"../../util/style/au-regional.xsl\"?>";
			
			String XmldtdVersionDeclaration = "<tga_ectd schema-version=\"3.0\"  xmlns=\"tga_ectd\"  xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"  xsi:schemaLocation=\"tga_ectd ../../util/dtd/au-regional.xsd\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" >";
			...
			out.write("\n");
			out.write("<esub-id>"
					+ publishForm.geteSubmissionId()
					+ "</esub-id>");
			out.write("\n");
			out.write("<applicant>" + publishForm.getApplicant()
					+ "</applicant>");
	
			String[] ann_au = publishForm.getAAN().split("#");
			...
			String[] multiproduct = publishForm.getProdName().split("#");
			
			
			for (int j = 1; j < multiproduct.length; j++) {
			
					out.write("\n" +"<product-name>" +  multiproduct[j]
				          + "</product-name>");	 
				
			}
			
			
			String[] argtNo = publishForm.getArtgNumber()!=null?publishForm.getArtgNumber().split("#"):null;
			
			if(argtNo!=null && argtNo.length >0){
				
				for(int i=1;i<argtNo.length;i++){
				
					out.write("\n" +"<artg-number>" + argtNo[i]
							+ "</artg-number>");	 
				}
			}
			out.write("\n" + "<sequence-type ");
			out.write("code-version=\"3.0\"");
			out.write(" code=\"" + publishForm.getSequenceType() + "\"");
			out.write("/>");
			
			out.write("\n" + "<reg-activity-lead ");
			out.write("code-version=\"3.0\"");
			out.write(" code=\"" + publishForm.getRegActLead() + "\"");
			out.write("/>");
			
			
			out.write("\n");
			out.write("<sequence-number>" + publishForm.getSeqNumber()
					+ "</sequence-number>");
			boolean flag=false;
			String[] seqdesDtl={"seq-desc-4","seq-desc-5","seq-desc-22"};
			String[] desvalue=publishForm.getSequenceDescriptionValue().split("#");
			for(int i=0;i<seqdesDtl.length;i++){
				if(seqdesDtl[i].equals(publishForm.getSequenceDescription())){
					flag=true;
					break;
				}
			}
			if(flag==true){
				...
				out.write("code-version=\"3.0\"");
				out.write(" code=\"" + publishForm.getSequenceDescription() + "\">");

				out.write("\n" + "<data ");
				out.write("use=\"date\">");
				out.write(desvalue[0].replaceAll("/","-") + " </data>");
				
				out.write("</sequence-description>");
			}
			else if( publishForm.getSequenceDescription().equals("seq-desc-21") || publishForm.getSequenceDescription().equals("seq-desc-24")){
				...
				out.write("\n" + "<data ");
				out.write("use=\"description\">");
				out.write(desvalue[0] + "</data>");
				
				out.write("</sequence-description>");
			}
			
			else if( publishForm.getSequenceDescription().equals("seq-desc-19")){
				...
				out.write("\n" + "<data ");
				out.write("use=\"number\">");
				out.write(desvalue[0] + "</data>");
				out.write("\n" + "<data ");
				out.write("use=\"date\">");
				out.write(desvalue[1].replaceAll("/","-") + " </data>");
				out.write("</sequence-description>");
			}
			
			else if( publishForm.getSequenceDescription().equals("seq-desc-20")){
				...
				out.write("\n" + "<data ");
				out.write("use=\"from-date\">");
				out.write(desvalue[0].replaceAll("/","-") + "</data>");
				out.write("\n" + "<data ");
				out.write("use=\"to-date\">");
				out.write(desvalue[1].replaceAll("/","-") + "</data>");
				out.write("</sequence-description>");
			}
			else{
			...
			out.write("\n" + "</au-envelope>");// End of envelopes
		}

		else if (stype.equals("aum2-m5")) {
			...
			String aumd5 = md5.getMd5HashCode(absolutePathToCreate + "/m1/au"
					+ "/au-regional.xml");
			System.out
					.println("CheckSum ="
							+ "<leaf xml:lang=\"en\" checksum-type=\"md5\" modified-file=\"\" operation=\"new\" application-version=\"\" xlink:href=\"m1/au/au-regional.xml\" checksum=\""
							+ aumd5
			...
			out
					.write("<leaf xml:lang=\"en\" checksum-type=\"md5\" operation=\"new\" application-version=\"\" xlink:href=\"m1/au/au-regional.xml\" checksum=\""
							+ aumd5
							+ "\" ID=\"node-999\" xlink:type=\"simple\">");
			out.write("<title>");
			out.write("AU Module 1");
