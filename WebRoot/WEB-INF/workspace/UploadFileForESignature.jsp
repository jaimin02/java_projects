...
function changeBtnName(){
		   ...
		   for (var i = 0; i < children.length; ++i)
			{
		 		 ...
		 		 for(var j=0; j<child.length; j++){
					...
					{
	 					alert("Invalid Document Name. \n\nOnly Alphabets, Digits,Dot,Underscore and Dash are allowed.");
      					document.workspaceNodeAttrForm.files1.style.backgroundColor="#FFE6F7"; 
      					return false;  			
					}
			  	}
   			}
   ...
   /*  	
	...
	function saveFile(){
        ...
        //var reffile =$('#uploadRefFile')[0].files[0];
        var reffile =$('#files1')[0].files;
        if(RefrenceFileUpload=="Yes" && reffile!=undefined){
        	
      		/* var reffilename = $('#uploadRefFile')[0].files[0].name
      		fd.append('uploadRefFile', reffile);
        	fd.append('RefFileName',reffilename); */
        	

       	 var reffilename = [] ;
       	  var totalfiles = document.getElementById('files1').files.length;
       	   for (var index = 0; index < totalfiles; index++) {
       		   fd.append("files1", document.getElementById('files1').files[index]);
       		   reffilename[index] = document.getElementById('files1').files[index].name;
       	   }
     		/* var reffilename = $('#files1')[0].files[0].name
     		fd.append('uploadRefFile', reffile);*/
       	fd.append('RefFileName',reffilename) ;
        }
        ...
        if(DMSvalidate()){
    		//debugger;
        $.ajax({
            url: 'SaveFileForESignature_ex.do',
            data: fd,
            type: "POST",
            contentType: false,
            processData: false,
            success: function(data) {
              //alert(data);
              if(data=="true"){
            	  var out="uploadFileForESignautre.do?docId="+wsId+"&recordId="+nodeId+"&docType="+docType;
          		  location.href=out;
              }
              else{
            	  alert(data);
	...
	function updateList() {
		debugger;
	    var input = document.getElementById('uploadRefFile');
	    var children = "";
	    var children = [];
	    for (var i = 0; i < input.files.length; ++i) {
	        children[i] = input.files.item(i).name;
	    }
	    document.getElementById('uploadRefFile').value=children;
	}
...
<div id="imp" style="float: right; margin-right: 10px; margin-top: -10px;">
	...
	<%-- <s:if test="isCreate==true && #session.usertypename=='WU' && ((stageId==10 && sendForReviewFlag==true) || stageId==0) "> --%>
	<s:if test="(#session.usertypename=='WU' || #session.usertypename=='WA') && (currentStageId==0 || currentStageId==10)">
		...
		<s:if test='RefrenceFileUpload =="Yes"'>		
			<tr id="fileTr">
				<td align="right" width="50%" style="padding: 2px; padding-right: 8px;">
					<span class="title">Reference Document</span>
					<span style="font-size:20px;color:red"></span> </td>
					<td align="left">
					<!-- <input type="file" id="uploadRefFile"> -->
	...
	<tbody>
		...
		<s:iterator value="fullNodeHistory" status="status">
			<tr class="<s:if test="#status.even">even</s:if><s:else>odd</s:else>">
				...
				<td style="border: 1px solid black;"><s:if test ="stageId==10 ">Uploaded</s:if>
					<s:else>
