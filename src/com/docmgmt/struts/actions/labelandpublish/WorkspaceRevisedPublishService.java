...
public class WorkspaceRevisedPublishService {
...
public String fname;
	...
	public WorkspaceRevisedPublishService() {
							...
							if (operationValue.equals("delete")) {
								...
								if (nodetypeindi.trim().equals("I")) {
									...
									attrValue = copyFileforPublish(publishForm,
											newPath);
									FileManager
									// Remove Directory when nodetypeindi="G"
									// otherwise empty directory will publish.
									.deleteDir(new File(pathToCreate));
								}/*else if(nodetypeindi.trim().equals("F") && nodeName.trim().equalsIgnoreCase("datasets")){
									
									String newPath=publishForm.getPublishDestinationPath()+"/"+publishForm.getSeqNumber()+"/m5/"+nodeName;
									attrValue = copyFileforPublish(publishForm,
											newPath);

									FileManager
											.deleteDir(new File(pathToCreate));
								}else if(nodetypeindi.trim().equals("S") && nodeName.trim().equalsIgnoreCase("datasets")){
									...
									FileManager
											.deleteDir(new File(pathToCreate));
									//String newPaths = pathToCreate.substring(0,pathToCreate.lastIndexOf("/"));
									System.out.println(pathToCreate);
									//FileManager
									//.deleteDir(new File(newPaths));
								}*/
								else
									attrValue = copyFileforPublish(publishForm,
											pathToCreate);
			...
			for (int i = 0; i < childNodes.size(); i++) {
				...
				if (AllNodesofHistory.contains(nodeId)) {
					...
					if (isLeaf == 0) {
						...
						// Parent nodes
						{
						...
						}/*else if(nodeName.equalsIgnoreCase("datasets")){
							
							//String actulaPaht=pathToCreate;
							...
							pathToCreate=pathToCreate.substring(0, newPath.length()+3);
	...
	private String getRootPath(int childNodeId, String path) throws Exception {
		...
		if (childNodeId != 1) {
			...
			DTOWorkSpaceNodeDetail dtowsndl = (DTOWorkSpaceNodeDetail) record
					.get(0);
			Integer ParentIdForChildNode = dtowsndl.getParentNodeId();

			Vector parentNodeDtl = docMgmtInt.getNodeDetail(wsId,
					ParentIdForChildNode.intValue());

			DTOWorkSpaceNodeDetail obj = (DTOWorkSpaceNodeDetail) parentNodeDtl
					.get(0);
			...
			// ParentIdForChildNode.intValue());
			Vector nodeAttribute = docMgmtInt.getNodeAttributes(wsId,
					ParentIdForChildNode.intValue());
			String MergeAttributeStr = "";
			if (nodeAttribute.size() > 0) {
				for (int k = 0; k < nodeAttribute.size(); k++) {
					DTOWorkSpaceNodeAttribute obj1 = (DTOWorkSpaceNodeAttribute) nodeAttribute
							.get(k);
	...
	private String copyFileforPublish(PublishAttrForm publishForm,
			DTOWorkSpaceNodeDetail dto) throws Exception {

		String absolutePath = "";
		String absolutePathLink = "";
		newPath = "";

		absolutePathLink = getRootPath(dto.getNodeId(), absolutePath);

		// System.out.println("absoluePathInCopyFile:"+absolutePathLink);

		// absolutePath = publishDestFolderName + File.separator + "ctd-" + wsId
		// +"/"+ publishForm.getSeqNumber() + absolutePathLink;
		// System.out.println("-----------------GetRootPath Done-------------");
		absolutePath = publishDestFolderName + File.separator
				+ publishForm.getApplicationNumber() + "/"
				+ publishForm.getSeqNumber() + absolutePathLink;

		fileNameDtl = docMgmtInt.getFileNameForNodeForPublish(dto.getNodeId(),
				wsId, publishForm.getLabelNo());
		folderNameDtl = docMgmtInt.getNodeDetail(wsId, dto.getNodeId());
		...
		if (fileNameDtl.size() > 0) {
			...
			if (folderNameDtl.size() > 0) {
				DTOWorkSpaceNodeDetail dtownd = (DTOWorkSpaceNodeDetail) folderNameDtl
						.get(0);

				fname = dtownd.getFolderName();
			}

			// folderStruct = new File(sourceFolderName.trim()
			// + folderStructure.trim() + "/" + fname);
			//
			...
			return (absolutePathLink.substring(1) + "/" + fname);
		} else {
	...
	private void writeToXmlFile(String stype, PublishAttrForm publishForm)
		...
		if (stype.equals("eu")) {
			...
			String XmldtdVersionDeclaration = "<eu:eu-backbone xmlns:eu=\"http://europa.eu.int\" xmlns:xlink=\"http://www.w3c.org/1999/xlink\" dtd-version=\"1.3\">";
			...
			if(publishForm.getEUDTDVersion().equals("14")){
			...
			else if(publishForm.getEUDTDVersion().equals("20")){
				...
				cmsDtl = docMgmtInt
						.getWorkspaceCMSSubmissionInfo(publishForm.getSubmissionId(),"13");
				// select * from View_WorkspaceCMSSubmissionDtl where SubmissionId =
				// '"+submissionId+"' order by CountryCodeName
			...
			if (publishForm.getRMSSelected() == 'Y') {
				...
				// ATC codes
				if (publishForm.getAtc() != null
						&& !publishForm.getAtc().equals("")) {
					String[] atcCodes = publishForm.getAtc().split(",");
					for (int iAtc = 0; iAtc < atcCodes.length; iAtc++) {
						out.write("\n" + "<atc>" + atcCodes[iAtc].trim()
								+ "</atc>");
					}
				}
				// Submission Type
				...
				// Invented-name
				out.write("\n" + "<invented-name>"
						+ publishForm.getInventedName() + "</invented-name>");
				// INNs
		...
		if (stype.equals("us")) {
			...
			/* Change for Manual Mode */
			if (dos == null || dos.equals("")) {
				dos = (String) request.getAttribute("dos");
			}
			String dos1[] = dos.split("/");
			String dosdt = dos1[0] + dos1[1] + dos1[2];

			out.write("\n" + "<date-of-submission><date format=\"yyyymmdd\">"
					+ dosdt + "</date></date-of-submission>");
			out.write("\n" + "</applicant-info>");
			...
			if (publishForm.getRelatedSeqNumber() != null
					&& !publishForm.getRelatedSeqNumber().equals("")) {
				String[] allRelatedSequences = publishForm
						.getRelatedSeqNumber().split(",");
				for (int i = 0; i < allRelatedSequences.length; i++) {
					out.write("\n" + "<related-sequence-number>"
							+ allRelatedSequences[i]
							+ "</related-sequence-number>");
				}
			}
			out.write("\n" + " </submission>");
