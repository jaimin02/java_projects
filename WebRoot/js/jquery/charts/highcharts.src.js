...
function Chart (options, callback) {
	...
	function MouseTracker (chart, options) {
		...
		function setDOMEvents () {
			...
			//This is click event for perticuler graph
			...
			{				
				...
				//alert(locationCode.value+productattrValue.value+dossierStatusattrValue.value);
				if(productattrValue.value!="-1" && dossierStatusattrValue.value!="-1"){
					chartBy="region";
				}else if(locationCode.value!="-1" && dossierStatusattrValue.value!="-1"){
					chartBy="product";
				}else{ 
					chartBy="dossier status";
				}					
				if (typeof(e)=="undefined")e=event;
				
				var height = $('#wsdetailtooltip1').height();
				var width = $('#wsdetailtooltip1').width();
				
				leftVal=e.clientX-(width+10)+"px";
				topVal=e.clientY+"px";
				$.ajax({									
					url: 'WorkSpaceDetailAction_ex.do?wsIdString='+hoverPoint.wsidstr+'&chartBy='+chartBy+'&othersValue='+hoverPoint.name,
					beforeSend: function()
					{ 
					$('#wsdetailtooltip2').html('');
					},			  		
					success: function(data) 
			  		{					
					$('#wsdetailtooltip1').css({left:leftVal,top:topVal,display:'block',visibility:'visible'}).show();
					
					$('#wsdetailtooltip2').html(data);
					//alert(document.getElementById("wsdetailtooltip1").style.zIndex);
					document.getElementById("wsdetailtooltip1").style.zIndex =99;					
					}							 
				});
				...
				if (!hasDragged) {
					if (hoverPoint && attr(e.target, 'isTracker')) {
						...
						//this one is for perticuler part
...
Point.prototype = {
	...
	applyOptions: function(options) {
		...
		// categorized data with name in first position
			...
			point.y = options[1];
			point.count = options[2];
			point.wsidstr = options[3];
	 ...
	 */
	//This is for click event when we click on perticuler part
