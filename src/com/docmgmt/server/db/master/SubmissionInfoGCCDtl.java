...
public class SubmissionInfoGCCDtl {
	DataTable datatable;
	public SubmissionInfoGCCDtl()
	{
	...
	public String insertSubmissionInfoGCCDtl(DTOSubmissionInfoGCCDtl dto,
		...
		try {
			...
			proc = con
					.prepareCall("{ Call Insert_SubmissionInfoGCCDtl(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");

			proc.setString(1, dto.getSubmissionInfoGCCDtlId());
			...
			if (rs.next()) {
				submissionInfoGCCDtlId = rs.getString(1);
			}
			proc.close();
	...
	public Vector<DTOSubmissionInfoGCCDtl> getWorkspaceSubmissionInfoGCCDtl(
			String workspaceId) {
		System.out.println("Ok");
		Vector<DTOSubmissionInfoGCCDtl> data = new Vector<DTOSubmissionInfoGCCDtl>();
		try {
			Connection con = ConnectionManager.ds.getConnection();
			ResultSet rs = null;

			rs = datatable.getDataSet(con, "*", "View_SubmissionInfoGCCDtl",
					"WorkspaceId='" + workspaceId + "'", "");
			System.out.println("Count");
			while (rs.next()) {
				DTOSubmissionInfoGCCDtl dto = new DTOSubmissionInfoGCCDtl();
				dto.setSubmissionInfoGCCDtlId(rs
						.getString("SubmissionInfoGCCDtlId"));
	...
	public DTOSubmissionInfoGCCDtl getWorkspaceSubmissionInfoGCCDtlBySubmissionId(
		...
		try {
			...
			rs = datatable.getDataSet(con, "*", "View_SubmissionInfoGCCDtl",
					"SubmissionInfoGCCDtlId='" + submissionId + "'", "");
			if (rs.next()) {
				dto.setSubmissionInfoGCCDtlId(rs
						.getString("SubmissionInfoGCCDtlId"));
				dto.setWorkspaceId(rs.getString("WorkspaceId"));
