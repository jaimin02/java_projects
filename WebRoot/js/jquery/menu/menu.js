...
(function($) {
    ...
    $.fn.lavaLamp = function(o) {
        ...
        return this.each(function(index) {
            ...
            //Custom Modification-Change for set cursor on current active url(parent)
            $(function() {
     	        var currentLoc = window.location.href;
     	         currentLoc = currentLoc.substring(currentLoc.lastIndexOf('/')+1, currentLoc.length);
    				
     	         	$('.menu a').each(function(){
	         	         var linkURL=$(this).attr('href').substring($(this).attr('href').lastIndexOf('/')+1);
	         	         if(linkURL=="#" && currentLoc.substring(0,16)=="WorkspaceOpen.do"){
	         	        	if($(this).find("span").text().toLowerCase()=="project")
     	        				setCurr($(this).parent()[0]);
	         	         }else if (currentLoc == linkURL){
	     	            	$currentMenuobj=$(this);
	     	            	$prevMenuObj=$currentMenuobj;
	     	            	do{
	     	            		$currentMenuobj=$currentMenuobj.parent();
	     	            		if($currentMenuobj.hasClass('menu'))
	     	            			break;
	     	            		$prevMenuObj=$currentMenuobj;
	     	            	}while(true);     	
	     	            	setCurr($prevMenuObj[0]);
	     	            }
	     	        });
     	      });
        });
