...
function trim(str)
{
	...
	function openlinkEdit(roleCode,clientName)
    {
    	
    	var editLocationWindow = "EditClient.do?roleCode="+roleCode+"&clientName="+clientName;
    ...
    function authenticate(roleCode,status)
	{
		if(status == 'D'){
			var okCancel = confirm("Do you want to active selected Role.");
		}else{
			var okCancel = confirm("Do you want to inactive selected Role.");
		}
		if(okCancel == true)
		{
			...
			if (remark != null && remark != ""){
				var revertWindow = "DeleteRole.do?roleCode="+roleCode+"&statusIndi="+status+"&remark="+remark;
	...
	function roleHistory(roleCode)
	{
		str="showRecordDetailHistory_b.do?roleCode=" + roleCode;
...
<div align="center"><!-- <img
	...
	<div class="boxborder"><div class="all_title"><b style="float:left">Role Master</b></div>

<div class="datatablePadding" style="border: 1px; border-color: #5A8AA9; border-style: solid; padding-bottom: 15px; border-top: none;"
	align="center"><br>
<div align="left"><s:form action="SaveRole" method="post" name="masterAdminForm">
	<table width="100%">
		<tr>
			<td class="title" align="right" width="50%" style="padding: 2px; padding-right: 8px;" id="clientName">Role</td>
...
<table id="clientTable" width="100%" align="center" class="datatable">
	...
	<tbody>
		<s:iterator value="roleDetail" id="clientDetail" status="status">
		...
		<s:if test ="#session.countryCode != 'IND'">
			...
			<div align=""><s:if test="statusIndi == 'E' || statusIndi == 'A'">
				<a title="InActivate" href="javascript:void(0);"
					onclick="authenticate('<s:property value="roleCode" />','<s:property value="statusIndi" />');">
			...
			<td style="border: 1px solid black;">
				<div align=""><a title="Client History" onclick="roleHistory('<s:property value="roleCode"/>');"
							href="#">
