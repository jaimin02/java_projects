...
$(document).ready(function() { 
	...
	//check if user have rights to sign off
	...
	var usercode = "<s:property value='#session.userid'/>";
	 $.ajax({		
			  url: "checkRightsForSignOff_ex.do?nodeId="+nodeId+"&workSpaceId="+wsId+"&usercode="+usercode,
			  success: function(data) 
			  {
				 //debugger;
				 temp = data;
				 if(data.includes("invalidSeq")){
					 window.stop();
					 var uName=data.split(":")[1];
				  	 alert("Please wait for "+uName+" to finish signoff activity.");
				  	 var out="ESignature.do?docId="+wsId+"&docType="+docType;
					 location.href=out;
				  	 return false;
				 }
				 if(data=="false"){
					 window.stop();
				  	 alert("You do not have rights to performe sign off.");
				  	 var out="ESignature.do?docId="+wsId+"&docType="+docType;
					 location.href=out;
				  	 return false;
				 }
			  ...
			  success: function(data) 
			  {
				 ...
				 if(data=="false"){
				  	alert("Please review document before sign off.");
				  	 var out="ChangeStatusDocSignForESignature.do?docId="+wsId+"&recordId="+nodeId+"&docType="+docType;
					 location.href=out;
				  	return false;
				 }
			  },
	...
	if(temp !="false"){
		...
		if(manualSignatureConfig=="Yes" && selectedStage!=0 && selectedStage!=30 && selectedStage!=50){
			...
			var win = window.open(applicationUrl+"ManualSignatureForESignature.do?workSpaceId="+'<s:property value="workspaceID"/>'+
		...
		if(selectedStage==30 || selectedStage==50){
	...
	if(sessPass==pass || sessAdUserPass==pass){
			...
			if(currStage==0 || currStage==30){
				var remark = prompt("Please specify reason for change.");
		    	 remark = remark.trim();
				 if(remark != null || remark != ""){ 
			 //$('#remark').val(remark);		
		$.ajax({		
			  url: "ChangeStatusEsignatureDocSign_ex.do?docId="+wsId+"&nodeId="+nodeId+"&stageCode="+selectedStageId+"&remark="+remark,
			  beforeSend: function()
			  { 
				 // debugger;
	...
	function sectionAttributes(workspaceID,nodeId){
		//alert("test");
		debugger;
		str="viewReferenceHistoryForESignature_b.do?WorkspaceId="+workspaceID+"&nodeId="+nodeId;
...
<s:if test="nodeHistoryForUser=='false' && stageCode!=0 ">
	<s:form name="workspaceNodeAttrForm" method="post" enctype="multipart/form-data" >
			...
			<td align="left">
			 <%--  <s:if test="stageuserdtl.size > 0"> 
				<s:select list="stageuserdtl" name="stageCode" listKey="stageIdForESIgn" id="stageCode"
					listValue="stageDesc" value="%{selectedStage}">
				</s:select>
	 		</s:if>
			<s:else>  --%>
			
			<s:if test="otsuka=='Yes'">
			<select id="stageCode" name="stageCode">
					<option value="100">Approve With Signature</option>
					<option value="0">Send Back</option>
					<option value="50">Approve Without Signature</option>
					<option value="30">Reject</option>
				</select>
				</s:if>
				<s:else>
			<select id="stageCode" name="stageCode">
					<option value="100">Sign Off</option>
					<option value="0">Send Back</option>
					<option value="50">Approve</option>
					<option value="30">Reject</option>
				</select>	
				</s:else>	
	...
	<thead>
		...
		<s:if test ="#session.countryCode != 'IND'">
			...
			<s:if test='RefrenceFileUpload =="Yes"'>
				<th style="border: 1px solid black;">Reference Document</th>
			</s:if>
		<%-- <th>Certified</th> 
		<th>Voided By</th>
		<th>Voided On</th>
	...
	<tbody>
		...
		<s:iterator value="fullNodeHistory" status="status">
			...
			<s:if test ="#session.countryCode != 'IND'">
				...
				<s:if test='RefrenceFileUpload =="Yes"'>
					<td style="border: 1px solid black;">
						<s:if test="RefFileName == null || RefFileName == '' ">
										-
						</s:if> 
						<%-- <s:if test="RefFileName == 'No File'">
							-<s:property value="RefFileName" />
						</s:if> <s:else>
						 <s:if test="OpenFileAndSignOff=='Yes'">
						 <s:if test="fileExt=='PDF' || fileExt=='pdf'">
						 	<a href="javascript:void(0);"
							onclick="return fileOpen('SignOffOpenfile.do?workSpaceId=<s:property value="workSpaceId"/>&nodeId=<s:property value="nodeId"/>&tranNo=<s:property value="tranNo"/>&fileName=<s:property value="RefFileName"/>&baseWorkFolder=<s:property value="baseWorkRefFolder"/>&fileExt=<s:property value="fileExt"/>');">
						<s:property value="RefFileName" /> </a>
						 </s:if>
						 <s:else>
						 	<a href="javascript:void(0);"
							onclick="return docFileOpen('SignOffOpenfile.do?workSpaceId=<s:property value="workSpaceId"/>&nodeId=<s:property value="nodeId"/>&tranNo=<s:property value="tranNo"/>&fileName=<s:property value="RefFileName"/>&baseWorkFolder=<s:property value="baseWorkRefFolder"/>&fileExt=<s:property value="fileExt"/>');">
						<s:property value="RefFileName" /> </a>
						 </s:else>
						 </s:if>
						 <s:else>
						  <s:if test="fileExt=='PDF' || fileExt=='pdf'">
							<a href="javascript:void(0);"
								onclick="return fileOpen('openfile.do?workSpaceId=<s:property value="workSpaceId"/>&nodeId=<s:property value="nodeId"/>&tranNo=<s:property value="tranNo"/>&fileName=<s:property value="RefFileName"/>&baseWorkFolder=<s:property value="baseWorkRefFolder"/>&fileExt=<s:property value="fileExt"/>');">
								<s:property value="RefFileName" /> </a>
						</s:if> --%>
						<s:else>
							<%-- <a href="javascript:void(0);"
								onclick="return docFileOpen('openfile.do?workSpaceId=<s:property value="workSpaceId"/>&nodeId=<s:property value="nodeId"/>&tranNo=<s:property value="tranNo"/>&fileName=<s:property value="RefFileName"/>&baseWorkFolder=<s:property value="baseWorkRefFolder"/>&fileExt=<s:property value="fileExt"/>');">
								<s:property value="RefFileName" /> </a> --%>
							<a><img id="imgPdf<s:property value="top[1]"/>" src="images/Common/details.svg" title="View History" height="25px" width="25px"  style=""
    							onclick="sectionAttributes('<s:property value="workSpaceId"/>',<s:property value="nodeId"/>);"/>
    						</a>
						</s:else>
					</td>
				</s:if>
