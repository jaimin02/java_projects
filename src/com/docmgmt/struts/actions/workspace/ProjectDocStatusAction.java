...
public class ProjectDocStatusAction extends ActionSupport {
	...
	public String execute()
	{
		...
		//ArrayList<DTOWorkSpaceNodeHistory> wsNodeHistory = docMgmtImpl.getAllNodesLastHistory(workspaceIds, nodeIds);
		  ...
		  for(int i=0;i<allStages.size();i++)
		  {
	    	  ...
	    	  if(key.equals(dtoStageMst.getStageId()))
	    	  {
	    		 ...
	    		 // if(!dtoStageMst.getStageDesc().equals("Send back"))
	    		  ...
	    		  //}
		    		  if(dtoStageMst.getStageDesc().equalsIgnoreCase("Approved") && mode==1){
		    			  String x = htmlContent;
		    			  System.out.println(x);
		    			  int firstPos = x.indexOf("PerLeafNodes=") + "PerLeafNodes=".length();
		    			  int lastPos = x.indexOf("#", firstPos);
		    			  String temp = x.substring(0,firstPos) + stgPerStr + x.substring(lastPos);
