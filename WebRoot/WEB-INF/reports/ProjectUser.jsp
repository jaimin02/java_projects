...
<script type="text/javascript">
	...
	function getProjectSnapshot(reportType){
		...
		var urlOfAction="ShowProjectUser_ex.do?mode="+mode;//mode=1  for latest  
		 ...
		 $.ajax({
			 ...
			 success:function(data)
			 {
				// debugger;
					...
					str+="&nbsp;&nbsp;&nbsp;Total Artifact/SubArtifact with files = "+ historyCount+"<br/>";
					str+="&nbsp;&nbsp;&nbsp;Total Artifact/SubArtifact without files = "+ (leafCount-historyCount)+"<br/>";
					tblstr+="<tr><td>Total Artifact/SubArtifact with files</td><td>"+historyCount+"</td></tr>";
					tblstr+="<tr><td>Total Artifact/SubArtifact without files</td><td>"+(leafCount-historyCount)+"</td></tr>";
					tblstr+="</table>";
	...
	{	
		...
		$.ajax(
		{			
			url: 'ShowProjectUser_ex.do?workSpaceId=' + workSpaceId+'&mode='+mode,
			beforeSend: function()
			{
					$('#ProjectSnapshotDiv').html("<img src=\"images/loading.gif\" alt=\"loading ...\" />");								
			},
...
function showNodeRemark(workspaceId,nodeId,ele){
	//debugger;
	var urlOfAction="ShowNodeRemark_ex.do";
	var dataofAction="workSpaceId="+workspaceId+"&nodeId="+nodeId;
	showSubDtl('NodeRemarkDiv',ele);
	$.ajax({
		type: "GET", 
			url: urlOfAction, 
			data: dataofAction, 
			...
			dataType:'html',
			beforeSend: function(){ 
			$('#NodeRemarkDiv').html('Loading...');
		},
		success: function(response){
			var javascript = "";
			$('#NodeRemarkDiv').html("<div align='center' class='datatable' style=' overflow:auto;max-height:115px; width:400px;background:white;'>"+response+"</div>");
		}
	});
...
<body><div id="backgroundPopup" onclick="proceed();"></div>
...
<br>
	...
	<input
			type="hidden" name="fileName" value="Assigned_User_On_Project_Report.xls">
		<textarea rows="1" cols="5" name="tabText" id="tableTextArea"
			...
			onclick="getTimeInfo();document.getElementById('tableTextArea').value=document.getElementById('statusDetails').innerHTML;;document.getElementById('myform').submit()">
...
<div align="center"><img
	src="images/Header_Images/Report/User_on_Project.jpg"
...
<div id="NodeRemarkDiv" style="display: none; z-index: 2000;"></div>
