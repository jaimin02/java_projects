...
public class vNeeSPublishAndSubmitAction extends ActionSupport {
	...
	public Vector<DTOSubmissionInfovNeeSDtl> getWorkspacePublishInfo;
	public Vector<DTOSubmissionInfovNeeSDtl> getWorkspaceConfirmDtl;
	...
	public String execute() throws Exception {
		...
		for (int i = 0; i < getAllWorkspace.size(); i++) {
			...
			if (dtoAllWorkSpaceMst.getProjectType() == ProjectType.VNEES_STANDARD) {
			...
			for (int k = 0; k < wsWithSubInfo.size(); k++) {
				...
				if (dtoAllWorkSpaceMst.getWorkSpaceId().equals(
						dtoSubWorkspaceMst.getWorkspaceId()) && dtoAllWorkSpaceMst.getProjectType() == ProjectType.VNEES_STANDARD) {
	...
	public String getSubmissionDtl() throws Exception {
		...
		if (wsdto.getProjectType() == 'N' || wsdto.getProjectType() == 'V') {
			...
			for (int i = 0; i < getWorkspacePublishInfo.size(); i++) {
				DTOSubmissionInfovNeeSDtl dtoNeeS0Dtl = (DTOSubmissionInfovNeeSDtl) getWorkspacePublishInfo
						.get(i);
			...
			getWorkspacePublishInfo = new Vector<DTOSubmissionInfovNeeSDtl>();
		}
	...
	public String saveSubmissionDtl() throws Exception {
		...
		DTOSubmissionInfovNeeSDtl submissionDtl = new DTOSubmissionInfovNeeSDtl();
		...
		submissionDtl.setSubmissionPath(currentSeqNumber+"/root-"+dtoWorkspace.getWorkSpaceDesc());// Setting
		...
		submissionId = docMgmtImpl
				.insertSubmissionInfovNeeSDtl(submissionDtl, 1);// Insert MODE
		
		docMgmtImpl.updatePublishedVersionForvNeeS(workSpaceId,
		...
		DTOSubmissionInfovNeeSDtl dtoNeeS = docMgmtImpl
	...
	public String confirmSubmission() {
		...
		if (result.equalsIgnoreCase("pathNotFound")) {
			...
			DTOSubmissionInfovNeeSDtl dtoNeeSDtl = docMgmtImpl
					.getWorkspaceSubmissionInfovNeeSDtlBySubmissionId(submissionInfo__DtlId);
			...
			dtoNeeSDtl.setSubmissionInfovNeeSDtlId(submissionInfo__DtlId);
			...
			docMgmtImpl.insertSubmissionInfovNeeSDtl(dtoNeeSDtl, 2); // Update
			// Published
			// Version
	...
	public void publishWorkspaceSubmission(DTOSubmissionMst dtoSubmissionMst,
		...
		vNeeSPublishService vneesPublishService = new vNeeSPublishService();
		vneesPublishService.setProjectPublishType(projectType);

		vneesPublishService.workspacePublish(workSpaceId, publishForm,
				wsDesc);
	...
	public Vector<DTOSubmissionInfovNeeSDtl> getGetWorkspacePublishInfo() {
		return getWorkspacePublishInfo;
	}

	public void setGetWorkspacePublishInfo(Vector<DTOSubmissionInfovNeeSDtl> getWorkspacePublishInfo) {
