...
<script type="text/javascript">
	...
	{ 
		...
		{
			target: '#msgDiv',
			url: 'SavevNeeSPublishAndSubmitForm.do',
...
<s:else>
	...
	<s:form action="SavevNeeSPublishAndSubmitForm" method="post"
				...
				<s:iterator value="getWorkspacePublishInfo" status="status">
					...
					<tr id="row<s:property value="#status.count"/>"
						...
						<td>${status.count}</td>
							...
							<td><s:property value="submissionInfovNeeSDtlId" /> <s:set
								name="subId" value="%{submissionInfovNeeSDtlId}"></s:set></td>
...
<!--						 && currentSeqNumber == #workspaceCurrentSeqNumber"-->
						...
						<td>
							...
							<s:if test="deleted == 'false' && submissionPath.startsWith('//')">
								<div align="center">	
								<a target="_blank" 
								href="file:<s:property value='submissionPath'/>/gtoc.pdf"
								 title="file:<s:property value='submissionPath'/>/gtoc.pdf">
								 <img
							...
							<s:elseif test="deleted == 'false' && submissionPath.charAt(1) == ':'">
								<a target="_blank" href="file:<s:property value='submissionPath'/>/gtoc.pdf" title="General TOC File">TOC</a>
							</s:elseif></td>
