...
//var $q = jQuery.noConflict(true);
	...
	$(document).ready(
			function() {
			    ...
			    document.getElementById('eCSVTd').style.display='none';	
				 $('.projectList').fSelect({
		    	    placeholder: 'Select User',
		    	    numDisplayed: 3,
		    	    overflowText: '{n} selected',
	...
	function ProjectType() {
		...
		var userCode ="<s:property value='userId'/>";
		var usergrpCode = "<s:property value='userGroupCode'/>";
		var ProjectCode=document.querySelector('#ProjectCode').value;
		...
		else if(ProjectCode=="0002"){// for eCSV projects
			$.ajax({
					url : 'GetWorkflowWiseProjectsizeReport_ex.do?userGroupCode='+usergrpCode+'&userId='+userCode,
					beforeSend : function() {
					...
					success : function(data) {
						$('#ProjectSnapshotDiv').html("");
						document.getElementById('eCSVTd').style.display='';
						var opt = "  ";			  		
					       if(data.length > 0)
						   {
					    	//debugger;
					    	 ...
					    	 for(i=0; i<usersArr.length;i++)
						       {
					    		...
					    		opt = '<select name="eCSVorkspace" id="eCSVorkspace" class="projectList" onchange="return subinfo();" '+
					    		opt + '</select> &nbsp;';
					    		 document.getElementById('showEcsvWorkspaceDtl').innerHTML = opt;
								}
					       		window.fs_test=$('.projectList').fSelect();
					       //$('#ProjectSnapshotDiv').html("<img src=\"images/loading.gif\" alt=\"loading ...\" />");
					       }
				});		
		}
		else{//for Esignature Workflow
			document.getElementById('eCSVTd').style.display='none';
			var workspaceid ="<s:property value='projectIdEsign'/>";
			$.ajax({
					url : 'ShowProjectsizeReport_ex.do?workSpaceId='+ workspaceid,
					beforeSend : function() {
						$('#ProjectSnapshotDiv').html("<img src=\"images/loading.gif\" alt=\"loading ...\" />");
					},
					success : function(data) {
	...
	function subinfo() {
		debugger;
		var e = document.getElementById("eCSVorkspace");
		var workSpaceId = e.value;
		if(workSpaceId=="-1"){
...
<body>
	<div class="container-fluid">
		<div class="col-md-12">
			...
			<div class="boxborder">
				<div class="all_title">
					<b style="float: left">Project Size Report</b>
				</div>
			</div>
			<!-- <img
				...
				<div align="center">
					...
					<table Style="width: 100%">
						<tr>
							...
							<td align="left">
							<s:select id="ProjectCode" list="projectTypes"
								name="projectCode" headerKey="-1" headerValue="Select Project Type" onchange="return ProjectType();"
						...
						<tr id="eCSVTd">
							...
							<td align="left">
								<div id="showEcsvWorkspaceDtl"></div>
							</td>
						</tr>
