...
.grid_clr table tr:nth-child(even) {background: #f5f5f5}
.grid_clr table tr:nth-child(odd) {background: #fff}
 */
...
<script type="text/javascript">
	...
	function updateHours(nodeId,userCode,userGroupCode,stageId){
		//debugger;
		...
		//alert("wsId="+workSpaceId+", NodeId="+nodeId+", UserCode="+userCode+", UserGroupCode="+userGroupCode+", StageId="+stageId);
		var timeDuration = document.getElementById("txt_"+nodeId+"_"+userCode+"_"+userGroupCode+"").value;
		 var isNumeric=timeDuration.match("^[0-9]*$");
		  if(!isNumeric){
			  alert("Please enter valid hours.");
			  document.getElementById("txt_"+nodeId+"_"+userCode+"_"+userGroupCode+"").style.backgroundColor="#FFE6F7"; 
			  document.getElementById("txt_"+nodeId+"_"+userCode+"_"+userGroupCode+"").focus();
		  }
		  else{
				if(timeDuration<=0){
				   alert("Please enter valid hours.");
			  	...
			  	$.ajax( {
					url:'UpdateProjectPlanningHours_ex.do?wsId='+workSpaceId+'&nodeId='+nodeId+'&userCode='+userCode+'&userGrpCode='
							+userGroupCode+'&duration='+timeDuration+'&stageId='+stageId,
					...
					error: function(data) 
					  {
						alert("Something went wrong while updating hours...");
					  },
					async: false
				}); 
				}
	...
	function getProjectSnapshot(reportType){
		...
		var urlOfAction="ProjectTimelineTracking_ex.do?mode="+mode;//mode=1  for latest  
	...
	{	
		...
		$.ajax(
		{			
			url: 'ProjectTimelineTracking_ex.do?workSpaceId=' + workSpaceId+'&mode='+mode,
			beforeSend: function()
			{
					$('#ProjectSnapshotDiv').html("<img src=\"images/loading.gif\" alt=\"loading ...\" />");								
			},
...
<div class="boxborder"><div class="all_title"><b style="float:left">Project Timeline Tracking</b></div></div><!-- <img
...
<table width="100%">
	<tr>
		 ...
		 <td><s:select id="workSpaceId" name="workSpaceId"
			...
			listValue="workSpaceDesc" onchange="getProjectSnapshot('TableView')"
			ondblclick="getProjectSnapshot('TableView')" headerKey="-1"
			headerValue="">
		</s:select> </td>
	</tr>
</table>
...
<label style="padding-left: 20px; font-size:14px"
	...
	name="h" value="Enter Text..." style="font-size:14px"
	...
	style="position: relative; bottom: 1px; background: transparent;">
<img alt="Next" src="images/Common/Next.png" onclick="findNxt();"
	title="Find Next" height="18px" width="18px"
	style="position: relative; bottom: 1px"></form>
