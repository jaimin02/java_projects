...
function openlinkEdit(buttonName,workSpaceID,stageIds,userCode,nodeId){
	...
	var roleCode = '<s:property value="roleCode"/>';
	//alert(stageIds);
	...
	$.ajax( {
		url:'RemoveAndUpdateRightsPopupFromHomePage_ex.do?workSpaceID='+workSpaceID+'&stageIds='+stageIds+'&userCode='+userCode+'&nodeId='+nodeId+'&remark='+remark+'&RightsType='+modulewiserights+'&uCode='+uCode+"&roleCode="+roleCode,
		beforeSend : function() {
...
<div style="border: 1px; border-radius: 10px 10px 0px 0px; border-color: #5A8AA9; border-style: solid; padding-bottom: 15px; border-top: none;"
	align="center">
	<div class="boxborder"><div class="all_title"><b style="float:left">Change Document Rights</b></div>
<br>
			...
			<td class="title" align="right" width="50%"
	 		...
	 		<td>
	 			<s:select list="assignWorkspaceRightsDetails"
					...
					listValue="loginName" theme="simple">
				</s:select>
			</td>
		</tr>
		<tr>
...
<div align ="center">
	...
	<input type="submit" value="Update Rights" class="button" name="buttonName"
			onclick="return openlinkEdit('Change Rights','<s:property value="WorkspaceId" />','<s:property value="stageId" />','<s:property	value="userCode" />','<s:property	value="nodeId" />','<s:property	value="roleCode" />');"  />
</div>
</div>
</div>
</div>
