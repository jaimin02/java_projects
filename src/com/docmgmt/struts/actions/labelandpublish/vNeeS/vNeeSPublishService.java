package com.docmgmt.struts.actions.labelandpublish.vNeeS;
...
public class vNeeSPublishService {

	private final float SRNO_COLUMN_WIDTH = 25f;
	private final int MAX_LEVEL_IN_TOC = 9;

	private static final String MAIN_TOC_NAME = "gtoc.pdf";
	private static final String TOC_TITLE = "Table Of Content";
	
	private static final int TOTAL_MUDULES = 5;
	
	public static final String NODE_NAME_3E_GMO="3e-gmo";
	public static final String NODE_NAME_3E_ANNEXES="3e-annexes";

	
	Font font = FontFactory.getFont("c:/windows/fonts/arial.ttf",
		    BaseFont.IDENTITY_H, 12, Font.BOLD, BaseColor.BLUE);
	Font fontForTableCell = FontFactory.getFont("c:/windows/fonts/arial.ttf",
		    BaseFont.IDENTITY_H, 12, Font.NORMAL, BaseColor.BLACK);
	...
	public void workspacePublish(String workspaceId,
		...
		try {
			...
			bf = BaseFont.createFont("c:/windows/fonts/arial.ttf", BaseFont.IDENTITY_H,BaseFont.NOT_EMBEDDED);
		
			System.out.println("IS embeded "+bf.isEmbedded());
			...
			if (projectPublishType == 'N' || projectPublishType == 'V') {
			...
			for (int currModuleNo = 1; currModuleNo < childs.size()+1; currModuleNo++) {
				...
				if (parentNodes.size() < 1
					...
					continue;
				String moduleTOCName="";
				
				 moduleTOCName = "p" + currModuleNo + "-toc.pdf";

				File moduleFolder = new File(absolutePathToCreate + "/p"
						+ currModuleNo + "/");

				String tocPath = "p" + currModuleNo + "/" + moduleTOCName;
				...
				createAndOpenDocument(absolutePathToCreate + "/p"
				...
				document.add(table);
				document.close();
				pdfutil.autoCorrectPdfProp(absolutePathToCreate + "/p" + currModuleNo + "/" + moduleTOCName,true);
					
				Collection data= CollectionUtils.retainAll(AllNodesofHistory,docMgmtInt.getNodeIdFromNodeName(wsId,NODE_NAME_3E_ANNEXES));

				//if file is uploded in annexxer node 
				
				if(data.size() >0 && currModuleNo==3){
					
						createP3eTOC(currModuleNo,publishForm,iParentId);
				}
			...
			if(AllNodesofHistory.size()<1 ){
				System.out.println("Document has no page.Please upload file in workspace.");
			}
			e.printStackTrace();
		}
	}
	public void createP3eTOC(int currModuleNo,PublishAttrForm publishForm,int ParentNodeId) throws IOException, DocumentException{
		Vector<DTOWorkSpaceNodeDetail> childs=docMgmtInt.getChildNodeByParent(ParentNodeId, wsId);
		parentNodes = docMgmtInt.getModuleWiseChildNodeByParentForNeesPublish(iParentId, childs.get(currModuleNo-1).getNodeNo(), wsId);
		DTOWorkSpaceNodeDetail dto = (DTOWorkSpaceNodeDetail) parentNodes.get(0);
		String moduleP3eTOCName="";
		String tocPath_gmo="";
		if(currModuleNo==3)
		{
					...
					&& AllNodesofHistory.contains(dto.getNodeId()))
			{
					...
					moduleP3eTOCName = "p" + currModuleNo + "e-toc.pdf";
					tocPath_gmo = "p" + currModuleNo + "/" +"3e-gmo/"+ moduleP3eTOCName;
					File moduleFolder_gmo = new File(absolutePathToCreate + "/p3/3e-gmo"+ "/");
					addCellinMainTOC("p3e-toc", tocPath_gmo);
					if (!moduleFolder_gmo.exists()) {
						moduleFolder_gmo.mkdirs();
					}
					createAndOpenDocument(absolutePathToCreate + "/p"
							+ "3" + "/"+"3e-gmo/" + moduleP3eTOCName);
					initTable();
					try {
						
						// to generate p3etoc.pdf inside gmo folder
						
						getp3eTOCNode(parentNodes, absolutePathToCreate, iParentId, 0,
								stype, publishForm, 0);
					} catch (Exception e) {
						// TODO Auto-generated catch block
						System.out.println("Error inside annexe method");
						e.printStackTrace();
					}
					addBlankCell();
			...
			//moduleP3eTOCName = "p" + currModuleNo + "e-toc.pdf";
			pdfutil.autoCorrectPdfProp(absolutePathToCreate + "/p3/3e-gmo/" + moduleP3eTOCName,true);
		}
	...
	private void addLinkCell(String nodeDisplayName, int level, String p3e) {
		...
		if(p3e != null && !p3e.equals("")){
			 action = new PdfAction(attrValue, 1);
		}
		else{
			 action = new PdfAction("../" + attrValue, 1);
		}
	...
	private void addCell(String nodeDisplayName, int level) {
		...
		try {
			...
			chunk.setFont(fontForTableCell);
			Chunk chunk1 = new Chunk(nodeName);
			chunk1.setFont(fontForTableCell);
			cell = new PdfPCell(new Phrase(chunk));
			table.addCell(cell);
	...
	public void getp3eTOCNode(Vector<DTOWorkSpaceNodeDetail> childNodes,
			String pathToCreate, int parentId, int IdValue, String stype,
			PublishAttrForm publishForm, int level) throws Exception {
			...
			if (AllNodesofHistory.contains(nodeId)) {
				...
				if (childAttrId.size() != 0) {
					for (int i = 0; i < childAttrId.size(); i++) {
						...
						attrName = dtowsnath.getAttrName();
						if (attrName.equalsIgnoreCase("xlink:href")) {
							if(nodeName.equalsIgnoreCase(NODE_NAME_3E_ANNEXES) || nodeName.equalsIgnoreCase(NODE_NAME_3E_GMO))	{
								if (nodetypeindi.trim().equals("I")) {
								String newPath = pathToCreate.substring(0,
										pathToCreate.substring(0,
												pathToCreate.lastIndexOf("/"))
							...
							attrValue = attrValue.toLowerCase();
							if(nodeName.equalsIgnoreCase(NODE_NAME_3E_ANNEXES))
							{
								splitPath = attrValue.substring(0,attrValue.substring(0,attrValue.lastIndexOf("/")).lastIndexOf("/"));
								attrValue = attrValue.substring(splitPath.length()+1,attrValue.length());
								attrValue = attrValue.replaceAll("//", "/");
								
							}
							if( nodeName.equalsIgnoreCase(NODE_NAME_3E_GMO))
							{
								splitPath = attrValue.substring(0,attrValue.lastIndexOf("/"));
								attrValue = attrValue.substring(splitPath.length()+1,attrValue.length());
								attrValue = attrValue.replaceAll("//", "/");
							...
							addLinkCell(nodeDisplayName, level - 1,"p3e");
					...
					if(nodeName.equalsIgnoreCase(NODE_NAME_3E_ANNEXES) || nodeName.equalsIgnoreCase(NODE_NAME_3E_GMO))
					{
					attrValue = copyFileforPublish(publishForm, pathToCreate,"p3e");
					attrValue = attrValue.toLowerCase();
					attrValue = attrValue.replaceAll("//", "/");
					addLinkCell(nodeDisplayName, level - 1,"p3e");
					}
			...
			for (int i = 0; i < childNodes.size(); i++) {
				...
				if (AllNodesofHistory.contains(nodeId)) {
					...
					if (isLeaf == 0) {
						...
						if(!nodeName.equalsIgnoreCase("specific") && !nodeName.equalsIgnoreCase("pi-doc")){
							if(nodeName.equalsIgnoreCase("p3")){
								addCell("p3e-toc p3e-toc table of content", level);
							}
							if(nodeName.equalsIgnoreCase(NODE_NAME_3E_ANNEXES) || nodeName.equalsIgnoreCase(NODE_NAME_3E_GMO))
							{
								addCell(nodeDisplayName, level);
					...
					if (!nodeName
							...
							.equals(
									"m1-administrative-information-and-prescribing-information")) {
							if(nodeName.equalsIgnoreCase(NODE_NAME_3E_ANNEXES)){
								relativePathToCreate=absolutePathToCreate+"/p3/3e-gmo/3e-annexes";
							}
							if(nodeName.equalsIgnoreCase(NODE_NAME_3E_GMO)){
								relativePathToCreate=absolutePathToCreate+"/p3/3e-gmo";
							}
						...
						if (isLeaf == 0) {// This is a parent node
							if (!nodetypeindi.trim().equalsIgnoreCase("T")
								...
								parentNodes = docMgmtInt.getChildNodeByParent(
										nodeId.intValue(), wsId);
								getp3eTOCNode(parentNodes, relativePathToCreate,
										nodeId.intValue(), IdValue, stype,
										publishForm, level + 1);
							}
						} else {
						...
						parentNodes = docMgmtInt.getChildNodeByParent(nodeId
								.intValue(), wsId);
						getp3eTOCNode(parentNodes, relativePathToCreate, nodeId
								.intValue(), IdValue, stype, publishForm,
								level + 1);
					}
...
// main else end
	...
	private String copyFileforPublish(PublishAttrForm publishForm,
			String pathToCreate,String p3eTOC) throws Exception {
		...
		if (fileNameDtl.size() > 0) {
			...
			if(p3eTOC=="" && p3eTOC.equalsIgnoreCase("") )
			{
