...
function openAssignUser(nodeId){
	...
	str="AttachUserOnRecordForESignature_b.do?docId="+wsId+"&recordId="+nodeId;
...
function deleteNode(wsId,nodeId)
{
	...
	var remark = prompt("Please specify reason for change.");
	 ...
	 if(remark == null || remark == ""){
		...
		window.location.href = "DeleteNodeForDocSingForESignature.do?WorkspaceId="+wsId+"&nodeId="+nodeId+"&remark="+remark+"&docType="+docType;
}
...
function downloadPdf(wsId,nodeId){
	//debugger;
 	var URL='DownloadForESignature_ex.do';
 	var docType='<s:property value="docType"/>';
 	
 	$.ajax(
 	{			
 		url: 'ConvertAndDownloadForESignature_ex.do?WorkspaceId='+wsId+"&nodeId="+nodeId+"&docType="+docType,
...
function sectionAttributes(workspaceID,nodeId){
	//alert("test");
	debugger;
	str="viewHistoryForESignature_b.do?WorkspaceId="+workspaceID+"&nodeId="+nodeId;
...
function checkRights(){
	alert("You don't have rights to perform this activity.")
}
</script>
...
<div>
 <s:if test="#session.usertypename == 'WA' || #session.usertypename == 'WU' ">
	<a title="Add" style="float: left; margin-left: 10px;" href="OpenRecordForESign.do?docId=<s:property value="WorkspaceId"/>&docType=<s:property value="docType"/>">
		<input type="button" value="New" class="button" name="buttonName"/>
	</a>
</s:if> 
...
<table id = "usertable" class="datatable paddingtable remarktbl" style="width:100%;" align="center">
	<thead>
		...
		<%-- <s:if test="#session.usertypename == 'WU'">
			...
			<th style="border: 1px solid; border-color:black;">History</th>
		</tr>
	...
	<tbody>
	 	<s:iterator value="getAttributeDetailsForDisplay" status="stat">
		 	  ...
		 	  <s:iterator value="%{top}" status="outerStatus">
		 		<s:if test="#outerStatus.first == true ">
		 		  <s:iterator status="status">
		 			<td style="border: 1px solid; word-wrap: break-word;word-break: break-word;"><s:property value="top" default="-"/></td>
		 		  </s:iterator>
		 		</s:if>
		 	  </s:iterator>
		 	</s:subset>		 	 
			<td style="border: 1px solid;"><s:property value="top[5]" /></td>
    		...
    		<td style="border: 1px solid;">
    		<s:if test= "(top[6] == #session.userid )"> 
				<a title="Assign Signatories" 
				 href="#" onclick="return openAssignUser('<s:property	value="top[1]" />');">
    				<img border="0px" title="Assign Signatories" alt="Assign Signatories" src="images/Common/assign_rights.svg" height="25px" width="25px"></a>
			 </s:if>
			<s:else>
				<img border="0px" title="Assign Signatories" alt="Assign Signatories" src="images/Common/assign_rights.svg" height="25px" width="25px">
			</s:else>     				
			 </td>
			...
			<s:if test= "(top[6] == #session.userid )">
				<a title="Edit <s:property value="docTypeName"/>"
				 href="EditAttributeForESignature.do?docId=<s:property value="top[0]"/>&recordId=<s:property value="top[1]"/>&docType=<s:property value="docType"/>" >
			...
			<td style="border: 1px solid;"> 
			 <%-- <s:if test="#session.usertypename == 'WU' && top[4] == 'true'"> --%> 
				...
				<a title="Upload Document"style="margin-top: 2px; margin-bottom: 3px" 
    		 		href="uploadFileForESignautre.do?docId=<s:property value="top[0]"/>&recordId=<s:property value="top[1]"/>&docType=<s:property value="docType"/>" >
    		...
    		<s:else><span>
    		 ...
    		 <%---<s:if test="#session.usertypename == 'WU'"> 
    		 <s:if test="top[4] == 'false' &&  #session.usertypename == 'WU'"> --%> 
    			<s:if test="top[8]=='true'">
    			<a title="Change Stage" style="margin-top: 2px; margin-bottom: 3px" 
    		 		href="ChangeStatusDocSignForESignature.do?docId=<s:property value="top[0]"/>&recordId=<s:property value="top[1]"/>&docType=<s:property value="docType"/>" >
    				...
    				<s:else>
    				<!-- <input type="button" value="Change Status" class="button" name="buttonName"/> -->
    				<img border="0px" alt="eSign" onclick= "return checkRights();"title ="eSign" src="images/Common/change_stage.svg" height="25px" width="25px"></a></s:else>
    		 <%-- </s:if>
    		<s:else><span style="">
				 	<img border="0px" alt="eSign" title ="eSign" src="images/Common/change_stage.svg" height="25px" width="25px">
				 </span></s:else>
			  </s:if> --%>
    		 </td> 
			...
			<td style="border: 1px solid;">
				<div class ="downloading" id="downloading<s:property value="top[1]"/>" style="width:10px;">
					<img height="25px" width="25px" src="images/loading.gif" alt="loading ..." />
				</div>
    				<a><img id="imgPdf<s:property value="top[1]"/>" src="images/Common/details.svg" title="View Hisatory" height="25px" width="25px"  style=""
    						onclick="sectionAttributes('<s:property value="top[0]"/>',<s:property value="top[1]"/>);"/>
    				</a>
    		</td>
			</tr>
		</s:iterator>		
